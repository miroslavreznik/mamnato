    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
      ...
      <div className="space-y-3">
        {relief.items.map((item) => (
          <div key={item.key} className="p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
            <div className="flex justify-between items-baseline gap-3">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{item.label}</span>
              <span className="text-right shrink-0">
                <span className="font-bold text-gray-900 dark:text-white">{fmt(item.monthly)} Kč</span>
                <span className="text-xs text-gray-400">/měs</span>
              </span>
            </div>
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{item.how}</p>
